...
import {
  ...
  type RawSnapshotRetentionClient,
  cleanupRawSnapshots,
} from "./raw-snapshot-retention";
import { RAW_SNAPSHOT_CONTENT_STATUSES, type RawSnapshotContentStatusValue } from "./raw-snapshot";

const NOW = new Date("2026-05-31T00:00:00.000Z");

describe("raw snapshot retention cleanup", () => {
  ...
  it("dry-runs expired metadata and leaves rows and files untouched", async () => {
    ...
    const client = new FakeRawSnapshotRetentionClient([
...
class FakeRawSnapshotRetentionClient implements RawSnapshotRetentionClient {
  ...
  rawSnapshot = {
    ...
    }) as RawSnapshotRetentionClient["rawSnapshot"]["findMany"],
    deleteMany: async ({
      where,
    }: Parameters<RawSnapshotRetentionClient["rawSnapshot"]["deleteMany"]>[0]) => {
