...
- `/price-report settings`：開啟私密設定面板，可查看每日價格報告狀態，並用選單調整統計區間、分類篩選與報告內容類型；「全部分類」代表不限制分類，取消任一分類後會改成只追蹤目前選取的分類。
- 「時間與上限」會開啟 modal，設定最多列出商品數與每日私訊發送時間；`time` 使用台北時間 `HH:mm`。報告內容至少包含降價、漲價或新增商品其中一種。
