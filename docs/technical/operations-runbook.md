...
- `/public-report status/manage/test`：管理伺服器公開價格報告，分別用於查看狀態、開啟設定面板與發送測試報告；此指令以 Discord command 權限限制可見性，只有具備管理伺服器權限的成員通常會看到。頻道設定寫入 `discord_public_price_report_settings`，不再由主機端環境變數指定。公開報告目標頻道需允許 bot 傳送訊息與嵌入連結。
