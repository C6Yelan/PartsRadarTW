...
| Discord 通知分工已定案 | Webhook 保留給 public 價格變動廣播與 admin smoke 告警；Discord bot 才負責使用者 slash command 與個人化通知。Bot 第一輪做 `/price-report settings/now`、`/watch`、`/watchlist`、`/unwatch` 與必要的通知發送，不做網站帳號、公開暴露個人追蹤清單、Discord 內保存配單、庫存通知或購買建議。 |
