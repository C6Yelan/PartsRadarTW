...
| Discord 價格變動報告分成 public 與 personal | Public 價格報告由 `/public-report` 在 Discord 伺服器內設定發送頻道，`discord-bot` 掃描 scheduled crawl 後尚未送出的本輪價格變動並送到已啟用頻道，列出商品名稱、站內商品連結、舊價、新價與差額，並以 `DISCORD_PRICE_REPORT_MAX_ITEMS` 控制上限；Discord bot 的 `/price-report now` 在指令所在頻道或私訊 context 以 embed 回覆中文報告，只為有資料的「價格變動」或「新增商品」產生 embed，摘要與價格變動方向標題使用 Markdown emphasis，商品列以單行呈現關鍵價格與站內商品連結，且小分類已顯示品牌時不再重複商品名稱開頭品牌；`/price-report settings` 的按鈕與 modal 開放每日 DM 報告，依使用者設定的 window / max_items / 台北時間發送時間、分類、商品名稱關鍵字與內容類型列出特定時間段內實際變價商品。 |
