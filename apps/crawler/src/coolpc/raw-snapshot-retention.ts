...
export interface RawSnapshotRetentionClient {
  rawSnapshot: {
...
export interface CleanupRawSnapshotsOptions {
  client: RawSnapshotRetentionClient;
  storageDir: string;
  now?: Date;
...
export type PrismaRawSnapshotRetentionClient = Pick<PrismaClient, "rawSnapshot">;
...
export function cleanupRawSnapshotsWithPrisma(
  ...
  return cleanupRawSnapshots({
    ...options,
    client: options.client as unknown as RawSnapshotRetentionClient,
  });
}
