...
| 第二版以資料可信度與價格洞察為主 | 第二版範圍聚焦價格歷史、價格變動探索、商品連結健康檢查、正式更正 / 移除流程、輕量規格副標、一次性商品比較與營運監控。 |
| 使用者導向 Discord bot 不列入第二版 | 第二版只保留管理者 / 維運者異常通知作為營運監控方向，不做使用者價格通知 bot。 |
...
| 第一版移除請求採人工處理流程 | 第一版不建立管理後台、自動審核系統或公開刪除 API。收到合理請求後，先以人工方式移除或停用受影響資料 / 圖片，並記錄商品 ID、來源 URL、採取動作與是否需補永久 override / parser 規則，避免後續匯入流程重新帶回已移除內容。 |
| 公開前移除機制採永久 override | 公開前最低實作不做後台、不做公開刪除 API，也不直接硬刪商品歷史。改以資料庫永久 override / removal table 記錄商品、圖片或來源連結的停用狀態與原因；公開 API/UI 必須排除或 fallback；crawler、image backfill 與 link checker 必須讀取此清單並略過受影響內容。圖片快取清理由 idempotent ops script 依 product ID / image key 執行。 |
...
| Discord bot 價格通知 | 第二版不做使用者導向 Discord bot；管理者 crawler 異常通知屬於營運監控，不等同使用者價格通知。 |
| 原價屋商品分類與規格整理規則優化 | 需等資料擷取與第一批商品資料觀察後再細化。 |
| 解析失敗資料管理介面 | 第一版不做後台管理介面；parse error 先只作為 crawler 除錯與人工檢查資料。 |

## 待決定

目前沒有已知會阻塞第一版公開前規劃的產品決策。新增待決定事項前，應先確認是否已由第二版 Roadmap 或技術文件吸收。
