...
| Discord 價格變動報告分成 public 與 personal | Public webhook 由 `crawler-daemon` 在本輪有商品價格變動時送出公開變價清單，列出商品名稱、站內商品連結、舊價、新價與差額，並以 `PRICE_CHANGE_DISCORD_MAX_ITEMS` 控制單輪上限；Discord bot 的 `/price-report now` 在指令所在頻道或私訊 context 回覆中文報告，分成「價格變動」與「新增商品」兩區；後續定期 personal price report 需依使用者設定的 interval / window / scope 與發送目標列出特定時間段內實際變價商品。 |
