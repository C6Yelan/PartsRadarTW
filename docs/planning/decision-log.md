...
| Discord 通知分工已定案 | Webhook 保留給 public 價格變動廣播與 admin smoke 告警；Discord bot 才負責使用者 slash command 與個人化通知。Bot 第一輪做 `/price-report settings/now`、統合新增 / 查看 / 編輯 / 移除的 `/watch` 管理介面與必要的通知發送，不做網站帳號、公開暴露個人追蹤清單、Discord 內保存配單、庫存通知或購買建議。 |
| Discord 價格變動報告分成 public 與 personal | Public webhook 由 `crawler-daemon` 在本輪有商品價格變動時送出公開變價清單，列出商品名稱、站內商品連結、舊價、新價與差額，並以 `PRICE_CHANGE_DISCORD_MAX_ITEMS` 控制單輪上限；Discord bot 的 `/price-report now` 在指令所在頻道或私訊 context 以 embed 回覆中文報告，只為有資料的「價格變動」或「新增商品」產生 embed，摘要與價格變動方向標題使用 Markdown emphasis，商品列以單行呈現關鍵價格與站內商品連結，且小分類已顯示品牌時不再重複商品名稱開頭品牌；`/price-report settings` 的按鈕與 modal 開放每日 DM 報告，依使用者設定的 window / max_items / 台北時間發送時間、分類、商品名稱關鍵字與內容類型列出特定時間段內實際變價商品。 |
