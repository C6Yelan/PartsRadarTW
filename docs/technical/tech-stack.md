...
| 項目 | 決策 | 說明 |
| --- | --- | --- |
| Repo 型態 | 單一 repo | 第一階段網站、API、crawler、資料模型與文件都放在同一個 repo。Discord bot 未來若加入，初期也先放同一個 repo；只有在部署、權限或維護成本明顯需要時才考慮拆 repo。 |
| 主要語言 | TypeScript | 第一階段網站、API、crawler 與未來 Discord bot 都以 TypeScript 為主，讓資料型別、驗證邏輯與工具鏈能在單一 repo 內保持一致。 |
...
| 資料庫 | PostgreSQL | 第一版使用 PostgreSQL 作為主要資料庫，支援商品主檔、價格歷史、目前價格、crawler 狀態與未來價格提醒功能。 |
...
| lint / format | Next.js 預設 ESLint | 第一版先沿用 Next.js 預設 ESLint 設定，不額外導入 Prettier 或其他格式化工具；若實作過程出現格式不一致或 CI 需求，再另行補充。 |
